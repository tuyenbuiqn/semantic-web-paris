namespace Common.Constants
{
    public static class MapConstants
    {
        public const string DefaultFullAddress = "75 Đinh Tiên Hoàng, Tràng Tiền, Hoàn Kiếm, Hà Nội, Vietnam";
        public const string DefaultAddress = "75";
        public const double DefaultLatitude = 21.039396;
        public const double DefaultLongtitude = 105.839908;
        public const int DefaultNationId = 33;
        public const int DefaultProvinceId = 37;
        public const int DefaultDistrictId = 149;
        public const int DefaultWardId = 184;
        public const int DefaultStreetId = 24;
    }
    public static class MapCategoryConstants
    {
        //public const int AccommodationFacility = 1;
        //public const int Restaurant = 2;
        //public const int RepresentativeOfficeLuuTru = 3;
        public const int Language = 4;
        public const int MedicalFacility = 5;
        public const int MedicalService = 6;
    }

    public static class MapIconConstants
    {
        //http://fontawesome.io/icons/
        //fontawesome.markers.HOTEL
        /*
         fontawesome icon: hotel => constants = fontawesome.markers.HOTEL
         fontawesome icon: book => constants = fontawesome.markers.BOOK
         */
        // Icons
        //public const string Hotel = "M9.216-27.575h62.208q.936 0 1.62.684t.684 1.62v16.128h-9.216v-9.216h-55.296v9.216h-9.216v-43.776q0-.936.684-1.62t1.62-.684h4.608q.936 0 1.62.684t.684 1.62v25.344zm20.736-11.52q0-3.816-2.7-6.516t-6.516-2.7-6.516 2.7-2.7 6.516 2.7 6.516 6.516 2.7 6.516-2.7 2.7-6.516zm43.776 9.216v-2.304q0-5.724-4.05-9.774t-9.774-4.05h-25.344q-.936 0-1.62.684t-.684 1.62v13.824h41.472z";
        //public const string BuildingO = "M13.824-17.207v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm0-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm9.216 0v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm27.648 18.432v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm27.648 18.432v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm27.648 18.432v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm18.432 9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216-9.216v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm9.216 0v2.304q0 .468-.342.81t-.81.342h-2.304q-.468 0-.81-.342t-.342-.81v-2.304q0-.468.342-.81t.81-.342h2.304q.468 0 .81.342t.342.81zm-9.216 49.536h13.824v-55.296h-41.472v55.296h13.824v-8.064q0-.468.342-.81t.81-.342h11.52q.468 0 .81.342t.342.81v8.064zm18.432-57.6v59.904q0 .936-.684 1.62t-1.62.684h-46.08q-.936 0-1.62-.684t-.684-1.62v-59.904q0-.936.684-1.62t1.62-.684h46.08q.936 0 1.62.684t.684 1.62z";
        //public const string Car = "M17.28-25.271q0-2.376-1.692-4.068t-4.068-1.692-4.068 1.692-1.692 4.068 1.692 4.068 4.068 1.692 4.068-1.692 1.692-4.068zm1.296-11.52h36.576l-3.204-12.852q-.072-.288-.504-.63t-.756-.342h-27.648q-.324 0-.756.342t-.504.63zm49.392 11.52q0-2.376-1.692-4.068t-4.068-1.692-4.068 1.692-1.692 4.068 1.692 4.068 4.068 1.692 4.068-1.692 1.692-4.068zm5.76-3.456v13.824q0 .504-.324.828t-.828.324h-3.456v4.608q0 2.88-2.016 4.896t-4.896 2.016-4.896-2.016-2.016-4.896v-4.608h-36.864v4.608q0 2.88-2.016 4.896t-4.896 2.016-4.896-2.016-2.016-4.896v-4.608h-3.456q-.504 0-.828-.324t-.324-.828v-13.824q0-3.348 2.358-5.706t5.706-2.358h1.008l3.78-15.084q.828-3.384 3.744-5.67t6.444-2.286h27.648q3.528 0 6.444 2.286t3.744 5.67l3.78 15.084h1.008q3.348 0 5.706 2.358t2.358 5.706z";
        //public const string Home = "M50.688-28.727v17.28q0 .936-.684 1.62t-1.62.684h-13.824v-13.824h-9.216v13.824h-13.824q-.936 0-1.62-.684t-.684-1.62v-17.28l.018-.108.018-.108 20.7-17.064 20.7 17.064q.036.072.036.216zm8.028-2.484l-2.232 2.664q-.288.324-.756.396h-.108q-.468 0-.756-.252l-24.912-20.772-24.912 20.772q-.432.288-.864.252-.468-.072-.756-.396l-2.232-2.664q-.288-.36-.252-.846t.396-.774l25.884-21.564q1.152-.936 2.736-.936t2.736.936l8.784 7.344v-7.02q0-.504.324-.828t.828-.324h6.912q.504 0 .828.324t.324.828v14.688l7.884 6.552q.36.288.396.774t-.252.846z";

        // Scales
        public const double DefaultScale = 0.3;

        // Fill Colors
        public const string IconColorBlack = "black";
        public const string IconColorRed = "red";
        public const double DefaultFillOpacity = 0.7;


        // Stroke
        public const double DefaultStrokeWeight = 0.1;
        public const string DefaultStrokeColor = "black";
        public const double DefaultStrokeOpacity = 0.7;


        public const int DefaultIconSizeWidth = 32;
        public const int DefaultIconSizeHeight = 37;

        public const double DefaultIconAnchorX = 0;
        public const double DefaultIconAnchorY = 32;

        public const double DefaultIconOriginX = 0;
        public const double DefaultIconOriginY = 0;

    }
    public static class MapTypeConstants
    {
        public const string Roadmap = "roadmap";
        public const string Satellite = "satellite ";
        public const string Hybrid = "hybrid";
        public const string Terrain = "terrain";
    }

    public static class MapFeatureTypeConstants
    {
        //https://developers.google.com/maps/documentation/javascript/style-reference
        public const string All = "all";

        public const string Administrative = "administrative";
        public const string AdministrativeCountry = "administrative.country";
        public const string AdministrativeLandParcel = "administrative.land_parcel";
        public const string AdministrativeLocality = "administrative.locality";
        public const string AdministrativeNeighborhood = "administrative.neighborhood";
        public const string AdministrativeProvince = "administrative.province";

        public const string Poi = "poi";
        public const string PoiAttraction = "poi.attraction";
        public const string PoiBusiness = "poi.business";
        public const string PoiGovernment = "poi.government";
        public const string PoiMedical = "poi.medical";
        public const string PoiPark = "poi.park";
        public const string PoiPlaceOfWorship = "poi.place_of_worship";
        public const string PoiSchool = "poi.school";
        public const string PoiSportsComplex = "poi.sports_complex";


        public const string Transit = "transit";
        public const string TransitStationBus = "transit.station.bus";

    }

    public static class MapVisibilityConstants
    {
        public const string On = "on";
        public const string Off = "off";
        public const string Simplified = "simplified";
    }
}
