...
(function(undefined) {
    ...
    var scannerLaser = Q(".scanner-laser"),
        ...
        play = Q("#btnScanQrCode"),
       // scannedImg = Q("#scanned-img"),
        scannedQR = Q("#txtQrCodeValue"),
        //grabImg = Q("#grab-img"),
    ...
    var args = {
        ...
        resultFunction: function(res) {
           ...
           // scannedImg.src = res.imgData;
            ...
            $("#txtQrCodeValue").val(res.code);
            $("#btnCertifyQrCode").removeClass("disabled-button");
            CertifyQrCode();
            decoder.stop();
        },
        getDevicesError: function (error) {
            console.log(error);
            var p, message = "Lỗi tham số thiết bị:\n";
            for (p in error) {
                message += p + ": " + error[p] + "\n";
            }
            alert(message);
        ...
        getUserMediaError: function (error) {
            console.log(error);
            var p, message = "Lỗi tham số UserMedia:\n";
            for (p in error) {
                message += p + ": " + error[p] + "\n";
            }
            alert(message);
